"""Work Order routes: OS CRUD, Kanban, Statistics"""
...
from models import (
    OSCreate, OSUpdate, OSStatus, OSTipo, Criticidade,
...
async def os_estatisticas(user: Dict = Depends(get_current_user)):
    ...
    prioridade_counts = {}
    for p in Criticidade:
        prioridade_counts[p.value] = await db.ordens_servico.count_documents({**query, "prioridade": p.value})
    ...
    return {
        "por_status": status_counts, "por_tipo": tipo_counts, "por_prioridade": prioridade_counts,
...
async def create_os(data: OSCreate, user: Dict = Depends(get_current_user)):
    ...
    os_doc = {
        ...
        "organization_id": org_id, "tipo": data.tipo.value, "origem": data.origem.value,
        ...
        "tempo_execucao_minutos": None, "observacoes": None, "fotos": [], "pecas_utilizadas": [],
    ...
    if data.responsavel_id:
        await criar_notificacao(data.responsavel_id, org_id, NotificacaoTipo.OS_ATRIBUIDA, f"Nova OS atribuída: #{numero}", f"Ativo: {ativo.get('tag', '')} - {data.titulo}", f"/os/{os_id}")
    os_doc.pop('_id', None)
    return os_doc
